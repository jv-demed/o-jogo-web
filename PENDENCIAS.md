...
- [x] ~~**Alvos de toque e semântica**~~ — varridos os **11** `onClick` que estavam em `<div>`/`<span>`/`<ICONS.*>`; hoje **todo `onClick` do repositório está num `<button>`**. Vale a lista, porque o item original só citava 3 lugares: `PageHeader`, `CardNavigation` (2), `ListCollection`, `Card`, loja, toggle lista/grade da coleção, lista de decks, e no editor de deck o `+`, o `×` e o chevron de salvar. Alvo mínimo de 48px (`h-12`/`w-12`), `aria-label` onde o conteúdo é só ícone, e `focus-visible:ring-[#e2d4b8]` em todos.
  - Dois casos não eram só semântica, eram **defeito de fato**: `<ICONS.add onClick>` e `<ICONS.close onClick>` no editor de deck penduravam o handler no `<svg>`, então a área clicável era o desenho do ícone, não a faixa colorida que aparenta ser o botão.
  - Nas pontas, os botões do `CardNavigation` ficam `disabled` com o ícone `invisible`, e não removidos: saem da ordem de foco sem a carta pular de lugar.
- [x] ~~**Interação inconsistente**~~ — unificado no **clique simples**, e o long-press foi removido do `Card` (prop `onLongPress`, `longPressDelay` e os 5 handlers de mouse/touch). Zero consumidores restantes.
  - **Por que o clique ganhou:** o long-press existia na grid da coleção e no editor de deck, provavelmente para não abrir o detalhe sem querer ao rolar. Mas um `<button>` de verdade já resolve isso — o navegador **não dispara `click` quando o toque vira rolagem**. Chegamos a escrever o cancelamento por `touchmove` à mão antes de perceber que era trabalho que a plataforma já fazia.
  - Isso apagou ~30 linhas do `Card`: sumiram `useRef`, `useState` e o `setTimeout`, e o feedback de press virou `active:brightness-75` — CSS, sem estado de React e sem re-render por toque.
  - No editor de deck o clique na carta abre o detalhe e o botão abaixo (`+`/`×`) adiciona ou remove — duas ações separadas, sem gesto escondido. **Ganho de teclado:** antes o detalhe da carta só abria por long-press, ou seja, era inalcançável sem mouse ou toque.
  - `Card` sem `onClick` (o visor do `CardNavigation`) continua sendo `<div>`: um `<button>` ali seria focável e não faria nada.
