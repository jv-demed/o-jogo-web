...
- **Schema dedicado `o_jogo`**, colunas em snake_case. Migrations 0001–0007 aplicadas. A 0009 (assentos de bot) já está aplicada. Falta aplicar, na ordem: a 0008 (`users.is_dev`, para as ferramentas de dev — depois de rodá-la, marque o seu jogador com `update o_jogo.users set is_dev = true where name = '<você>'`) a **0010** (`matches.state` e `match_commands`), a **0011** (o log de comandos, abaixo), a **0012** (o id da partida vindo do relógio), a **0013** (partida com cheats) e a **0014** (a sala vazia se apaga).
...
- **O lobby, fechado em 2026-08-28.** Três decisões que não se deduzem do código:

  **O id da partida vem do relógio** (migration 0012), e não mais de uma sequence: `AAMMDDHHMM` + dois dígitos sorteados de desempate, no fuso de São Paulo — `260828204317`. Sequence tem uma propriedade ruim para um número que a pessoa lê e digita: ela não volta, então toda sala desfeita deixa um buraco permanente e o próximo id anuncia quantas tentativas houve antes dele. Continua `bigint`: nenhuma FK mudou de tipo, nenhuma RPC mudou de assinatura, e as partidas antigas seguem válidas com os ids pequenos que já tinham. O desempate é sorteado para duas partidas seguidas não saírem vizinhas.

  **Entrar é pelo link, e o link agora existe em algum lugar.** A RLS não deixa ser de outro jeito — `matches_read_participant` só mostra partida de que o jogador já participa, então não há `select` capaz de procurar a sala de outra pessoa. O host compartilha (`InviteBox`, com cópia e WhatsApp), e quem chega deslogado não perde o convite: o middleware manda o destino em `?next=`, e a tela de login volta para lá depois de entrar (só caminho absoluto deste site, senão o login viraria redirecionamento para fora).

  **A tela inicial oferece a sala aberta, quando há uma só** (`o_jogo.open_lobby`, em `supabase/manual/open_lobby.sql`). Não contradiz o item acima: continua não havendo `select` que ache a sala alheia, e quem responde é uma RPC `security definer` que devolve o mínimo do convite — id, nome e cara do host, quantos já estão sentados. Havendo **duas ou mais** salas abertas ao mesmo tempo, ela devolve vazio de propósito: escolher por conta própria mandaria gente para a mesa errada, e quem tem duas salas no ar tem o link para desempatar. Ficam de fora também a mesa cheia (o `join_match` recusaria, e o menu teria oferecido porta fechada) e a sala com mais de duas horas — o id carrega o relógio de quando ela nasceu, e sala velha é quase sempre aba fechada sem soltar assento. A home pergunta a cada 15s e ao voltar para a aba, e não escuta realtime: o evento de `matches` não chega a quem a RLS ainda não deixa ver a linha.

  **Sala vazia não fica no banco** (migration 0014). Saiu o último humano de uma partida que **nunca começou**, a partida vai embora e os bots dela junto — partida que não começou não tem log nem `initial_state`, e não reproduz nada. O corte é exatamente `initial_state is null`, e é ele que protege o replay: o log de uma partida jogada continua valendo depois que todo mundo foi embora da tela. Sair virou RPC (`leave_match`) por causa do host: o clique dele precisa encerrar a sala e soltar o assento na mesma transação, senão a aba fechada no meio deixaria a sala meio cancelada.
...
- **Ferramentas de dev** (`domain/match/dev.js`, `components/dev/`): quem tem `o_jogo.users.is_dev` vê a alça **DEV** no canto da mesa e um botão ao lado de "Comprar carta" para escolher a carta que vem. Hoje dá para escolher a compra, pôr carta direto na mão, revelar missões e mãos dos bots, pausar os bots (com passo a passo) e fechar a janela de interferência na marra.
  ...
  ⚠️ **Poder de dev nunca vira comando do motor.** Não existe `Command.devDraw`: um comando de dev aceito pelo `apply` seria trapaça de verdade no multijogador. Os poderes são funções puras em `domain/match/dev.js` que recebem o estado e devolvem outro, e por isso `domain/match/index.js` **não** reexporta esse arquivo. Escolher a compra *empilha o baralho* e deixa o `Command.draw` normal acontecer, em vez de inventar uma compra especial: a jogada de teste percorre o caminho real. Toda cirurgia entra no `state.log` como `dev.*` e aparece narrada no log da partida.

  **O painel saiu do solo (migration 0013).** Ele não podia sair enquanto a cirurgia acontecesse só na tela de quem clicou — por fora do `apply` nada era gravado, e a mesa do host andaria para um lado e a dos convidados para outro. O que destravou foi o log da 0011: o poder virou um comando `dev.*` (`DevCommand`/`applyDev`, em `dev.js`), viaja pela mesma fila da jogada, entra no log com `seq` e autor, e o `replayMatch` o refaz — refazer uma partida com cheats *sem* as cirurgias daria outra partida. Quem roteia é quem despacha: `applyDev` de um lado, `apply` do outro; o motor continua sem conhecer dev, que é o que o mantém seguro para rodar no servidor.

  Duas travas, as duas no banco, porque gate no bundle não autoriza nada: a partida precisa estar marcada `cheats` — e só o host marca, **só com 2+ devs na mesa**, porque poder de cirurgia sobre a mesa dos outros só vale entre pares que sabem o que estão testando — e o trigger `match_commands_check_dev` recusa comando `dev.*` fora de partida com cheats ou de quem não é dev. Na mesa, pausar bots é do host (`canDriveBots`): os bots são comandados por ele, e o botão na tela do convidado não pararia nada.
