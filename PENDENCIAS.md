...
- **O lobby, fechado em 2026-08-28.** Três decisões que não se deduzem do código:
  ...
  **Entrar é pelo link, e o link agora existe em algum lugar.** A RLS não deixa ser de outro jeito — `matches_read_participant` só mostra partida de que o jogador já participa, então não há como procurar a sala de outra pessoa. O host compartilha (`InviteBox`, com cópia e WhatsApp), e quem chega deslogado não perde o convite: o middleware manda o destino em `?next=`, e a tela de login volta para lá depois de entrar (só caminho absoluto deste site, senão o login viraria redirecionamento para fora).
